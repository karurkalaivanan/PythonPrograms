...
def readLoadProfileReading(serial_obj):
    try:
        ...
        try:
            ...
            todayDate1 = "/home/lpu/KPTCL_rs485/logs1/L.log"
    ...
    try:
        ...
        try:
            ...
            if(res == 1):
                ...
                print("----------------------------------------------------------------------------------------------------------------------------------")
                data = "$"+nodeid+"_"+str(PhaseType)+"_"+profilename+"_06_"+metermake+"_"+meterslno+"_"+finalstring[0:len(finalstring)-1]+"$"  
                todayDate1 = "/home/lpu/KPTCL_rs485/logs1/IP.log"
                append_to_file(todayDate1,data)   
                append_to_file(todayDate,data)   
...
def readSclarObisValues(serial_obj,profilename,obis):
    try:
        ...
        try:
            ...
            print("----------------------------------------------------------------------------------------------------------------------------------")
            data = "$"+nodeid+"_"+str(PhaseType)+"_"+profilename+"_06_"+metermake+"_"+meterslno+"_"+finalstring[0:len(finalstring)-1]+"$"  
            todayDate1 = "/home/lpu/KPTCL_rs485/logs1/"+meterslno+"_config_files.log"
            append_to_file(todayDate1,data)
            append_to_file(todayDate,data)  
            g_RRR = 0
            g_SSS = 0
    ...
    try:
        ...
        try:
            ...
            if(res == 1):
                ...
                print("----------------------------------------------------------------------------------------------------------------------------------")
                data = "$"+nodeid+"_"+str(PhaseType)+"_"+profilename+"_06_"+metermake+"_"+meterslno+"_"+finalstring[0:len(finalstring)-1]+"$"  
                todayDate1 = "/home/lpu/KPTCL_rs485/logs1/"+meterslno+"_billing.log"
                append_to_file(todayDate1,data)
                append_to_file(todayDate,data)  
...
def responseparsing(response,flag,iv1,profiletype):
    try:
        ...
        if(flag == "SNRM"):
            ...
            write_to_file("/home/lpu/KPTCL_rs485/config/meterslno_"+gwid,meterslno)
...
def readProfilesForConfigurations(serial_obj):
    try:
        ...
        # ////////////////////////////////////////////////////billing configurations/////////////////////////////////////////////////////////////////////
            ...
            #     esvpacket= ReadAllProfiles("ESV","01C1000701005E5B07FF0200")   #EventScalar 0x07, 0x01, 0x00, 94, 91, 0x07, 0xFF, 0x02, 0x00
            
            # elif(eob == 0):  
            
            #     eobpacket= ReadAllProfiles("EOB","01C100070000636201FF0300")   #EventdataObis 0x07, 0x00, 0x00, 99, 98, 0x01, 0xFF, 0x03, 0x00
            
            # elif(esob == 0):  
                
            #     esobpacket= ReadAllProfiles("ESOB","01C1000701005E5B07FF0300")   #Eventobisscalar 0x07, 0x01, 0x00, 94, 91, 0x07, 0xFF, 0x03, 0x00
...
def ReadOnDemandCommandsStatus(serial_obj):
    data = read_file("/home/lpu/KPTCL_rs485/config"+meterslno+"_OnDemandCommands")
    try:
        if(len(data) > 2):
            ...
            for command in commands:
                ...
                if(len(commandtoberead) > 2):
                    ...
                    delete_file("/home/lpu/KPTCL_rs485/config"+meterslno+"_OnDemandCommands")
...
def gettime():
    try:
        ...
        write_to_file("/home/lpu/KPTCL_rs485/config/curr_ver.txt",str(gw_current_firmware_version))
...
todayDate = "/home/lpu/KPTCL_rs485_test/logs/"+str(ist_time.strftime('%Y-%m-%d %H:%M:%S'))+".tmp"
finalfilename  = "/home/lpu/KPTCL_rs485_test/logs/"+str(ist_time.strftime('%Y-%m-%d %H:%M:%S'))+".log"


def getcommands():
    try:
        meterslno = read_file("/home/lpu/KPTCL_rs485/config/meterslno_"+gwid)
        url = "https://api.ms-tech.in/v11/getcommands?gwid="
        url = url + meterslno
        data = getData(url)
        print(data)
        if(len(data) > 5):
            data = write_to_file("/home/lpu/KPTCL_rs485/config"+meterslno+"_OnDemandCommands",data)
        return data
    except Exception as e:
        print(e)
...
gwid = read_file("/home/lpu/KPTCL/config/gwid.txt")
nodeid = gwid
def main():
    try:
