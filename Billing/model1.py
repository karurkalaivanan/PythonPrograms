...
class BillingViewer(tk.Tk):
    ...
    def load_file(self):
        ...
        try:
            ...
            new_data = parse_billing(apdu)

            # 🔥 Instead of replacing old data → append
            self.billing_data.extend(new_data)

            self.display_data(append=True)

            messagebox.showinfo("Success", "Billing data appended successfully!")
    ...
    def display_data(self, append=False):
        ...
        # If first time loading → create columns
        if not append or not self.tree["columns"]:

            self.tree.delete(*self.tree.get_children())

            max_columns = max(len(row) for row in self.billing_data)

            columns = [f"C{i}" for i in range(max_columns)]
            self.tree["columns"] = columns
            self.tree["show"] = "headings"

            for i, col in enumerate(columns):
                header_text = f"Parameter {i+1}"
        ...
        for row in self.billing_data[existing_rows:]:
